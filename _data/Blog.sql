...
CREATE TABLE IF NOT EXISTS `Blog`.`user` (
  ...
  `head_picture` VARCHAR(100) NOT NULL DEFAULT '' COMMENT '头像',
  ...
  `status` VARCHAR(100) NOT NULL DEFAULT '' COMMENT '用户状态',
...
CREATE TABLE IF NOT EXISTS `Blog`.`article` (
  ...
  `modify_time` INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '修改时间',
  `type` VARCHAR(100) NOT NULL DEFAULT '' COMMENT '文章类型，默认为0。0代表文章，1代表说说，2代表独立页面',
  `status` VARCHAR(100) NOT NULL DEFAULT '' COMMENT '状态',
  `home_page_show` TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '是否推荐到首页，0为否，1为是，默认为0',
...
CREATE TABLE IF NOT EXISTS `Blog`.`article_comments` (
  ...
  `status` VARCHAR(100) NOT NULL DEFAULT '' COMMENT '状态（是否验证通过，是否删除等）',
